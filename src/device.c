...
int wg_device_handle_post_config(struct net_device *dev, struct asc_config *asc)
{
	...
	if (wg->junk_size[MSGIDX_HANDSHAKE_INIT] ||
	    wg->junk_size[MSGIDX_HANDSHAKE_RESPONSE] ||
	    wg->junk_size[MSGIDX_HANDSHAKE_COOKIE] ||
	    wg->junk_size[MSGIDX_TRANSPORT] ||
	    wg->headers[MSGIDX_HANDSHAKE_INIT].start != MESSAGE_HANDSHAKE_INITIATION ||
	    wg->headers[MSGIDX_HANDSHAKE_INIT].end != MESSAGE_HANDSHAKE_INITIATION ||
	    wg->headers[MSGIDX_HANDSHAKE_RESPONSE].start != MESSAGE_HANDSHAKE_RESPONSE ||
	    wg->headers[MSGIDX_HANDSHAKE_RESPONSE].end != MESSAGE_HANDSHAKE_RESPONSE ||
	    wg->headers[MSGIDX_HANDSHAKE_COOKIE].start != MESSAGE_HANDSHAKE_COOKIE ||
	    wg->headers[MSGIDX_HANDSHAKE_COOKIE].end != MESSAGE_HANDSHAKE_COOKIE ||
	    wg->headers[MSGIDX_TRANSPORT].start != MESSAGE_DATA ||
	    wg->headers[MSGIDX_TRANSPORT].end != MESSAGE_DATA)
		a_sec_on = true;

	for (i = 0; i < ARRAY_SIZE(wg->ispecs); ++i) {
		if (wg->ispecs[i].desc && wg->ispecs[i].desc[0] != '\0')
			a_sec_on = true;
