...
bool client_id_asc_coexist(struct wg_device *wg)
{
	return
		wg->headers[MSGIDX_HANDSHAKE_INIT].start <= 0xFF &&
		wg->headers[MSGIDX_HANDSHAKE_INIT].end <= 0xFF &&
		wg->headers[MSGIDX_HANDSHAKE_RESPONSE].start <= 0xFF &&
		wg->headers[MSGIDX_HANDSHAKE_RESPONSE].end <= 0xFF &&
		wg->headers[MSGIDX_HANDSHAKE_COOKIE].start <= 0xFF &&
		wg->headers[MSGIDX_HANDSHAKE_COOKIE].end <= 0xFF &&
...
static void awg_restore_protected_type(struct sk_buff *skb,
				       struct wg_device *wg, __le32 protection_hash)
{
	struct message_header *header = (struct message_header *)skb->data;

	if (!client_id_asc_coexist(wg))
		return;
	header->type &= cpu_to_le32(0xFF);
	header->type |= protection_hash & cpu_to_le32(0xFFFFFF00);
}
	...
	if ((wg->random_trailers ?
		...
		if (awg_header_matches(skb, wg, &wg->headers[MSGIDX_HANDSHAKE_INIT],
			...
			if (wg->random_trailers &&
				...
				return 0;
			if (protected) {
				awg_restore_protected_type(skb, wg, protection_hash);
				if (!wg_header_protection_crypt(&wg->header_protection, nonce,
					...
					struct sk_buff *skb)
{
	...
	under_load = !wg->disable_cookies &&
		atomic_read(&wg->handshake_queue_len) >=
...
void wg_packet_receive(struct wg_device *wg, struct sk_buff *skb)
{
	...
	if (mh_validate(type, &wg->headers[MSGIDX_HANDSHAKE_INIT]) ||
		mh_validate(type, &wg->headers[MSGIDX_HANDSHAKE_RESPONSE]) ||
		mh_validate(type, &wg->headers[MSGIDX_HANDSHAKE_COOKIE])) {
		int cpu, ret = -EBUSY;
	...
	} else if (mh_validate(type, &wg->headers[MSGIDX_TRANSPORT])) {
